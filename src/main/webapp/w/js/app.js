controllers.controller('xdController', function($scope, $rootScope, common, modal, menu, $state, $filter, $mdSidenav, $location, configuration) {
    ...
    $rootScope.fillGrid = function(grid) {
        ...
        return function(data, total) {
            grid = grid ? grid : me.grid;
            if (!angular.isBlank(total)) {
                grid.data = data;
    ...
    $rootScope.loadPage = function(url, query) {
        var me = this;
        var conf;
        ...
        return function(page, limit) {
            common.loadPage(url, angular.extend({
                page: page,
                limit: limit
            }, conf), me.fillGrid());
        }
    ...
    $rootScope.createGridOption = function(columnDefs, conf, hold){
        var gridLoad = this.gridLoad;
        if (!gridLoad){
            gridLoad = this.loadPage(this.url,this.query);
        }
        var grid = common.createGrid(columnDefs,{
            scope: this,
            loadData: gridLoad,
